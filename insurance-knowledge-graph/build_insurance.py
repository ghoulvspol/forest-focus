#!/usr/bin/env python3
"""
Build a static HTML site for the Insurance Knowledge Graph.
Usage: python3 build_insurance.py
"""
...
VAULT = Path("/Users/harden/Documents/Obsidian Vault/保险业务体系/保险业务体系")
OUT = Path("/Users/harden/workspace/insurance-knowledge-graph/insurance-site")
SITE_TITLE = "保险业务体系知识库"
SITE_LOGO = "B"
BASE_PATH = "/insurance-knowledge-graph"  # GitHub Pages subdirectory, set to "" for root domain
...
# --bg:          light warm background
# --blue:        primary accent (deep blue)
# --blue-dark:   darker blue for hover
# --blue-glow:   subtle blue glow
# --dark:        deep navy for sidebar
# --dark-light:  lighter navy for sidebar elements

# ── Step 1-5: Core functions (same as template) ──────────────────────────────
...
def build_backlinks(files, link_map):
    ...
    for f in files:
        ...
        for m in re.finditer(r'\[\[([^\]|]+?)(?:\|[^\]]+)?\]\]', raw):
            ...
            if target_stem == src_stem or target_stem in seen_targets:
                continue
            ...
            backlinks[target_stem].append({
                "stem": src_stem, "category": src_cat,
                "title": src_title, "excerpt": excerpt,
            })
    return backlinks
...
.nav-changelog.active{color:var(--orange-light)}
...
.fav-btn:hover .fav-label{color:var(--orange)}
.fav-pop{
  display:none;
  position:absolute;
  right:0;top:48px;
...
.change-list li strong{color:var(--dark);font-weight:700}
...
.type-概念{background:#1E3A5F}
.type-产品{background:#0E7490}
.type-公司{background:#7C3AED}
.type-人物{background:#BE185D}
.type-方法{background:#059669}
.type-素材{background:#6B6560}
...
.article a{
  color:var(--link);
  text-decoration:none;
  background:linear-gradient(to bottom,transparent 60%,var(--orange-glow) 60%);
  transition:background .2s;
}
.article a:hover{background:linear-gradient(to bottom,transparent 40%,rgba(232,93,4,.2) 40%)}
...
.article blockquote a{background:none;color:var(--orange)}
...
.article th{background:var(--bg2);font-weight:600;color:var(--dark)}
...
.bl-action:hover{border-color:var(--orange);color:var(--orange)}
...
.bl-go-link:hover{color:var(--orange);text-decoration:underline}
...
.hero-sub b{color:var(--dark);font-weight:700}
...
.stat-item:hover{background:var(--orange-glow)}
.stat-num{
  font-family:var(--serif);
  font-size:42px;
  font-weight:900;
...
.nav-card:hover .nav-card-arrow{color:var(--orange);transform:translateX(4px)}
...
.tag.tier-1{
  ...
  font-weight:700;
  border-color:var(--orange-light);
  background:linear-gradient(135deg,#FFFDF5,#FFF3E8);
}
.tag.tier-1 .tag-n{font-size:12px;padding:3px 10px;background:var(--orange)}
...
.person-avatar{
  ...
  font-family:var(--serif);
  font-weight:700;
  box-shadow:0 4px 12px rgba(26,21,16,.15);
}
.person-name{
  font-family:var(--serif);
  font-size:15px;
...
.footer-promo::before{
  ...
  border-radius:50%;
  background:radial-gradient(circle,rgba(232,93,4,.15),transparent 70%);
}
...
.promo-credit strong{color:var(--orange-light);font-weight:700}
...
def build_sidebar_html(files, current_stem=""):
    ...
    for cat in ["sources", "concepts", "products", "companies", "people", "methods", "index-pages"]:
        groups[cat].sort(key=lambda x: x["stem"])

    home_active = " active" if current_stem == "欢迎" else ""
    html = f'<a href="{BASE_PATH}/index.html" class="nav-link nav-home{home_active}">\U0001f3e0 首页</a>\n'
    ...
    if has_changelog:
        ...
        html += f'<a href="{BASE_PATH}/index-pages/更新日志.html" class="nav-link nav-changelog{changelog_active}">\U0001f4cb 更新日志</a>\n'
...
<aside class="sidebar">
  <div class="sidebar-header"><a href="{BASE_PATH}/index.html" class="logo">保险业务体系知识库</a></div>
...
def build_backlinks_html(stem, backlinks_map, link_map):
    ...
    if not bl:
        return ""
    cat_labels = {
        "sources": "素材", "concepts": "概念", "products": "产品",
        "companies": "公司", "people": "人物", "methods": "方法", "index-pages": "索引",
    }
    items_html = ""
    for item in bl:
...
def build_homepage(files, link_map, ref_counts):
    ...
    product_files = [f for f in files if f["category"] == "products"]
    product_ranked = sorted(product_files, key=lambda f: ref_counts.get(f["stem"], 0), reverse=True)[:12]
    ...
    source_files = [f for f in files if f["category"] == "sources"]

    total_links = sum(ref_counts.values())
    arrow = "\u2192"

    # People data for avatar cards
    people_initials = {}
    for f in people_files:
        name = f["stem"]
        # Use first character as avatar
        people_initials[name] = (name[0], f["fm"].get("role", ""))
...
<section class="hero-section">
  <div class="hero-eyebrow">Insurance Business System</div>
  <h1 class="hero-title">保险业务体系<span class="orange">知识库</span></h1>
  <p class="hero-sub"><b>{len(concept_files)} 个</b>核心概念，<b>{len(product_files)} 个</b>保险产品，<b>{len(company_files)} 家</b>公司档案<br>
  从大数法则到智能核保——构建保险行业的全景知识图谱</p>
</section>
...
<div class="stats-row">
  ...
  <a href="{BASE_PATH}/index-pages/产品索引.html" class="stat-item"><div class="stat-num">{len(product_files)}</div><div class="stat-label">产品模型</div></a>
  <a href="{BASE_PATH}/index-pages/公司索引.html" class="stat-item"><div class="stat-num">{len(company_files)}</div><div class="stat-label">公司档案</div></a>
  <a href="{BASE_PATH}/index-pages/人物索引.html" class="stat-item"><div class="stat-num">{len(people_files)}</div><div class="stat-label">关键人物</div></a>
</div>

<div class="main-inner">
<div class="nav-cards">
  <a href="{BASE_PATH}/index-pages/核心概念索引.html" class="nav-card">
    <span class="nav-card-icon">\U0001f4a1</span>
    <div class="nav-card-title">核心概念</div>
    <div class="nav-card-sub">{len(concept_files)} 个保险核心概念</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="{BASE_PATH}/index-pages/产品索引.html" class="nav-card">
    <span class="nav-card-icon">\U0001f4e6</span>
    <div class="nav-card-title">产品模型</div>
    <div class="nav-card-sub">{len(product_files)} 个保险产品</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="{BASE_PATH}/index-pages/公司索引.html" class="nav-card">
    <span class="nav-card-icon">\U0001f3e2</span>
    <div class="nav-card-title">公司档案</div>
    <div class="nav-card-sub">{len(company_files)} 家保险公司</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="{BASE_PATH}/index-pages/人物索引.html" class="nav-card">
    <span class="nav-card-icon">\U0001f464</span>
    <div class="nav-card-title">关键人物</div>
    <div class="nav-card-sub">{len(people_files)} 位行业人物</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
</div>
...
<div class="section">
  <div class="section-header">
    <h2 class="section-title">产品模型</h2>
    <div class="section-line"></div>
    <span class="section-count">TOP ''' + str(len(product_ranked)) + '''</span>
  </div>
  <div class="tag-cloud">
'''
    for i, f in enumerate(product_ranked):
        url = link_map.get(f["stem"], "#")
        count = ref_counts.get(f["stem"], 0)
        tier = " tier-1" if i < 3 else (" tier-2" if i < 6 else "")
        html += f'    <a href="{url}" class="tag{tier}">{f["stem"]}<span class="tag-n">{count}</span></a>\n'
...
<div class="section">
  <div class="section-header">
    <h2 class="section-title">公司档案</h2>
    <div class="section-line"></div>
...
'''
    for f in people_files:
        url = link_map.get(f["stem"], "#")
        initial, role = people_initials.get(f["stem"], (f["stem"][0], ""))
        count = ref_counts.get(f["stem"], 0)
...
<div class="footer-promo">
  <div class="promo-story">
    <h3>关于本站</h3>
    <p>把保险行业的核心概念、险种产品、公司档案、关键人物整理成一张可以漫游的知识图谱。从大数法则到精算定价，从核保理赔到偿二代监管，覆盖保险业务的全景视图。</p>
    <p>独有视角：<strong>概念交叉引用</strong>（每个概念都链接到相关产品和公司）和 <strong>产品架构分析</strong>——帮助你建立保险行业的系统性认知。</p>
    <p class="promo-credit">本站是由滔哥与 <strong>Claude Code</strong> 共同完成的。</p>
  </div>
</div>
...
def main():
    ...
    assets_src = Path(__file__).parent / "assets-insurance"
