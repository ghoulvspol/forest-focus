...
from .buddy import BuddyCompanion, BuddyEvent, BuddyEventType
from .buddy.notification import notify_and_render
...
def build_parser() -> argparse.ArgumentParser:
    ...
    buddy_parser = subparsers.add_parser('buddy', help='interact with the companion (buddy) subsystem')
    buddy_parser.add_argument('--name', default='Clawd', help='companion name')
    buddy_parser.add_argument(
        '--event',
        default='session_started',
        choices=[e.value for e in BuddyEventType],
        help='event to fire',
    )
    buddy_parser.add_argument('--payload', default='', help='optional event payload')
    buddy_parser.add_argument('--frames', type=int, default=1, help='number of animation frames to render')
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == 'buddy':
        companion = BuddyCompanion.create(name=args.name)
        event = BuddyEvent(event_type=BuddyEventType(args.event), payload=args.payload)
        render = notify_and_render(companion, event)
        print(render.as_text())
        for _ in range(args.frames - 1):
            companion.advance_frame()
            extra = companion.render()
            print(extra.as_text())
        return 0
