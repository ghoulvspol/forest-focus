#!/usr/bin/env python3
"""
Build a static HTML site for the WWII Knowledge Graph from an Obsidian vault.
Usage: python3 build_wwii.py
...
VAULT = Path(__file__).parent / "vault" / "wwii"
...
# ── Step 6: HTML Template (Military Green Theme) ─────────────────────────────
...
.type-大事件{background:#8B2F2F}
.type-战役{background:#4A5D23}
.type-概念{background:#5E4A8B}
.type-国家{background:#1A6B7C}
.type-人物{background:#8B5E0B}
...
.article a:hover{background:linear-gradient(to bottom,transparent 40%,rgba(74,93,35,.2) 40%)}
...
.tag.tier-1{
  ...
  font-weight:700;
  border-color:var(--gold-light);
  background:linear-gradient(135deg,#FDFAF0,#F5F0E1);
}
...
.person-avatar{
  ...
  font-family:var(--serif);
  font-weight:700;
  box-shadow:0 4px 12px rgba(45,58,30,.15);
}
.person-name{
  font-family:var(--serif);
  font-size:15px;
...
.footer-promo::before{
  ...
  border-radius:50%;
  background:radial-gradient(circle,rgba(74,93,35,.2),transparent 70%);
}
...
def build_sidebar_html(files, current_stem=""):
    ...
    for cat in ["events", "battles"]:
        groups[cat].sort(key=lambda x: x["stem"])
    for cat in ["concepts", "nations", "people", "index-pages"]:
        groups[cat].sort(key=lambda x: x["stem"])
    ...
    for cat, label in order:
        ...
        group_items = [f for f in groups[cat]]
...
<aside class="sidebar">
  <div class="sidebar-header"><a href="/index.html" class="logo">二战知识图谱</a></div>
...
def build_homepage(files, link_map, ref_counts):
    ...
    battle_files = [f for f in files if f["category"] == "battles"]
    concept_files = [f for f in files if f["category"] == "concepts"]
    nation_files = [f for f in files if f["category"] == "nations"]
    ...
    battle_ranked = sorted(battle_files, key=lambda f: ref_counts.get(f["stem"], 0), reverse=True)[:8]
...
<section class="hero-section">
  <div class="hero-eyebrow">World War II · 1939-1945</div>
  <h1 class="hero-title">第二次世界大战<span class="gold">知识图谱</span></h1>
  <p class="hero-sub"><b>{len(event_files)} 个</b>大事件，<b>{len(battle_files)} 场</b>关键战役，<b>{len(concept_files)} 个</b>战略概念<br>
  从绥靖政策到柏林陷落——追踪人类最大规模战争的完整脉络</p>
</section>

<div class="stats-row">
  <a href="/index-pages/大事件索引.html" class="stat-item"><div class="stat-num">{len(event_files)}</div><div class="stat-label">大事件</div></a>
  <a href="/index-pages/战役索引.html" class="stat-item"><div class="stat-num">{len(battle_files)}</div><div class="stat-label">战役</div></a>
  <a href="/index-pages/概念索引.html" class="stat-item"><div class="stat-num">{len(concept_files)}</div><div class="stat-label">战略概念</div></a>
  <a href="/index-pages/国家索引.html" class="stat-item"><div class="stat-num">{len(nation_files)}</div><div class="stat-label">参战国</div></a>
  <a href="/index-pages/人物索引.html" class="stat-item"><div class="stat-num">{len(people_files)}</div><div class="stat-label">关键人物</div></a>
</div>

<div class="main-inner">
<div class="nav-cards">
  <a href="/index-pages/大事件索引.html" class="nav-card">
    <span class="nav-card-icon">📜</span>
    <div class="nav-card-title">大事件</div>
    <div class="nav-card-sub">从绥靖到投降</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="/index-pages/战役索引.html" class="nav-card">
    <span class="nav-card-icon">⚔️</span>
    <div class="nav-card-title">战役</div>
    <div class="nav-card-sub">{len(battle_files)} 场关键战役</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="/index-pages/概念索引.html" class="nav-card">
    <span class="nav-card-icon">💡</span>
    <div class="nav-card-title">战略概念</div>
    <div class="nav-card-sub">{len(concept_files)} 个核心概念</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="/index-pages/国家索引.html" class="nav-card">
    <span class="nav-card-icon">🌍</span>
    <div class="nav-card-title">国家阵营</div>
    <div class="nav-card-sub">轴心国 vs 同盟国</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="/index-pages/人物索引.html" class="nav-card">
    <span class="nav-card-icon">👤</span>
    <div class="nav-card-title">关键人物</div>
    <div class="nav-card-sub">{len(people_files)} 位决策者</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
</div>
...
<div class="section">
  <div class="section-header">
    ...
    <div class="section-line"></div>
    <span class="section-count">TOP ''' + str(len(battle_ranked)) + '''</span>
  </div>
  <div class="tag-cloud">
'''
    for i, f in enumerate(battle_ranked):
        url = link_map.get(f["stem"], "#")
        count = ref_counts.get(f["stem"], 0)
        tier = " tier-1" if i < 3 else (" tier-2" if i < 6 else "")
        html += f'    <a href="{url}" class="tag{tier}">{f["stem"]}<span class="tag-n">{count}</span></a>\n'
...
'''
    for f in people_files:
      ...
      <div class="person-refs">{role}</div>
    </a>
'''
...
<div class="footer-promo">
  <div class="promo-story">
    <h3>关于本站</h3>
    <p>把第二次世界大战散落在各类史书、档案、回忆录中的知识，整理成一张可以漫游的知识图谱。从 1938 年的绥靖政策到 1945 年的日本投降，跨越 7 年。</p>
    <p>涵盖 <strong>大事件</strong>、<strong>关键战役</strong>、<strong>战略概念</strong>、<strong>参战国家</strong> 和 <strong>历史人物</strong> 五大维度，每张卡片都通过 wikilinks 互联，构建完整的知识网络。</p>
    <p class="promo-credit">本站由 <strong>知乔</strong> 与 <strong>Claude Code</strong> 共同完成。</p>
  </div>
</div>
